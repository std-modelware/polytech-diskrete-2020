...
def check_symmetry (arr: list, p: int, size: int):
    ...
    for i in range(0, size, 1):
        ...
        if(l > (size - 1) / 2):
            flag = flag + [i]
    return flag
# flag = check_symmetry([1,1,0,0,0,0],1,6)
# flag = flag + check_symmetry([1,1,0,0,0,0],0,6)
# print(flag)

#Функция : изобразить график
#строка 57 - 74: изобразить ось симметрии
        ...
        if(flag != []):
            for i in range(0, len(flag), 1):
                plt.plot([(x[flag[i]] + x[(flag[i] + 1) % size]) / 2, -(x[flag[i]] + x[(flag[i] + 1) % size]) / 2],
                         [(y[flag[i]] + y[(flag[i] + 1) % size]) / 2, -(y[flag[i]] + y[(flag[i] + 1) % size]) / 2], color="y",
                         linewidth=0.5)
    plt.show()
        ...
        if(temp != []):
            if (n % 2 == 1):
                ...
                for j in range(0, len(temp), 1):
                    ...
                    f.write("\n")
        if(n % 2 == 0 and total[i].count(1) % 2 == 0):
            if(temp == []):
